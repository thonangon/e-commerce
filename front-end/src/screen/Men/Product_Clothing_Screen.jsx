  ...
  return (
    <VStack space={4} p={4} w="100%" maxW="400px" mx="auto" bg="white">
      <HStack alignItems="center" space={20} mb={4}>
        ...
        <Text bold fontSize={18} color="black">Clothings</Text>
      </HStack>
      <ScrollView>
        {subcategories.map((cloths) => (
          <Box key={cloths.id} flex="1" mt={2} p={4}>
            {cloths.categories.map((category) => (
              <VStack key={category.id} mb={4}>
                ...
                >
                  <HStack alignItems="center" space={3}>
                    <Image
                      source={category.image ? { uri: `http://10.0.2.2:8000${category.image}` } : require('../../assets/category_page.png')}
                      alt={category.name}
